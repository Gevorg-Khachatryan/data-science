...
def guess_the_genre(lyric):
    ...
    for item in list(df.index):
        coincidence = difflib.SequenceMatcher(None, df['lyrics'][item].lower(),
                                              lyric.lower()).ratio() * 100
        if coincidence > max_coincidence:
            max_coincidence = coincidence
            ind = item
    if ind:
        print("Most likely it is", df['genre'][ind])
    else:
        print("Sorry , i don't now")


lyric = """Look, I was gonna go easy on you and not to hurt your feelings
